...
- **Regression Suite** — 64 logical coverage flows for broader local/CI validation. Some IDs split into platform-specific variants or seed helper YAML files.
...
| REG-166 | `166-widget-card-settings.yaml` | `android-widget` | Android | In-app Widget Settings changes the global Article age dropdown from the default Last 3 days to Last 24 hours, selects Card layout, exposes card controls plus Divider/Image dependents, changes opacity/radius/separation/image sizing, then scrolls back to assert the preview container remains available. |
...
- **Real Cloud provider auth and most cloud mutations** — Mocked success/error auth, Google Drive mock state, Android/Dropbox unlink, Dropbox/Google Drive backup, and real provider sync need live credentials or provider-side mock support. Seeded linked screens, iOS iCloud unlink (REG-138), iOS iCloud backup (REG-142), and the pending-upload action (REG-133) are covered.
  Android Drive disconnect/reopen with a retained Google authorization grant is covered by Robolectric ViewModel tests, including delayed restore completion. Maestro cannot seed that provider grant independently of FeedFlow's local connection settings yet.
- **Cloud unread/bookmark reversions across devices** — Maestro has no isolated cloud byte store. The Gradle cloud harness exercises real repositories, platform workers and SQLite files for every flag transition, upload then sender/peer refresh, and stale-device refresh preserving newer remote data. See `docs/CLOUD_SYNC_TESTING.md`; no live cloud account is used.
- **Quiet cloud sync failures** — existing Maestro seeds cannot inject provider transfer/discovery failures. `FeedSyncMessageQueueTest` covers notification policy, reauthorization deduplication and explicit iCloud setup feedback; `CloudTransferRegressions` verifies that actual worker failures and later retries stay quiet while retaining pending edits. No production error-injection hooks are added for UI automation.
...
- **Android widget launcher placement and real iOS SpringBoard widget tap** — launcher-host placement remains a manual check because it needs stable widget-host / SpringBoard automation. Android in-app Widget Settings are covered by REG-166, and the app-owned widget feed-link route is covered by REG-126.
- **Widget preview row count, launcher freshness, and card visual parity** — REG-166 verifies only the app-owned Article age dropdown, Card controls, and preview-container availability. Maestro does not place or inspect launcher widgets, count preview rows reliably, or inspect colors, alpha, divider insets, Fill/Thumbnail geometry, or clipping. Deterministic preview rows and freshness filtering are covered by focused unit tests; launcher List/Card filtering, inclusive boundaries, empty state, and visual parity require physical widget-host validation.
...
- **RSS User-Agent fallback after HTTP 403** — WAF behavior varies by host, source IP, and time, while Maestro feed fixtures deliberately avoid live network requests. `RssParserWrapperTest` verifies the retry policy, and `RssParserFactoryTest` verifies the real JVM request sequence and headers. Validate the RSS fallback against CHIP when changing it; because the primary User-Agent is also used by the HTML retriever, changes to that value require a separate reader-mode check against Tweakers.
