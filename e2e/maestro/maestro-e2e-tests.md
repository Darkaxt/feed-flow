...
| REG-164 | `164-widget-card-settings.yaml` | `android-widget` | Android | In-app Widget Settings changes the global Maximum articles slider, selects Card layout, exposes card controls plus Divider/Image dependents, changes opacity/radius/separation/image sizing, then scrolls back to assert the preview container remains available. |
...
- **Widget preview row count and card visual parity** — REG-164 verifies the app-owned Maximum articles control, Card controls, and preview-container availability, but Maestro cannot assert that a limit of 1 renders one preview row or inspect colors, alpha, divider insets, Fill/Thumbnail geometry, or clipping. The one-row preview rule and renderer cap are covered by focused unit tests; manually inspect the dedicated Compose previews and validate launcher rendering when host automation is available.
