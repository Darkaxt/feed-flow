...
- **Regression Suite** — 62 logical coverage flows for broader local/CI validation. Some IDs split into platform-specific variants or seed helper YAML files.
...
| REG-164 | `164-widget-card-settings.yaml` | `android-widget` | Android | In-app Widget Settings changes the global Article age dropdown from the default Last 3 days to Last 24 hours, selects Card layout, exposes card controls plus Divider/Image dependents, changes opacity/radius/separation/image sizing, then scrolls back to assert the preview container remains available. |
...
- **Widget preview row count, launcher freshness, and card visual parity** — REG-164 verifies only the app-owned Article age dropdown, Card controls, and preview-container availability. Maestro does not place or inspect launcher widgets, count preview rows reliably, or inspect colors, alpha, divider insets, Fill/Thumbnail geometry, or clipping. Deterministic preview rows and freshness filtering are covered by focused unit tests; launcher List/Card filtering, inclusive boundaries, empty state, and visual parity require physical widget-host validation.
