...
| REG-164 | `164-widget-card-settings.yaml` | `android-widget` | Android | In-app Widget Settings selects Card layout, exposes card controls plus Divider/Image dependents, changes opacity/radius/separation/image sizing, then scrolls back to assert the preview container remains available. |
...
- **Widget card preview two-row geometry and visual parity** — REG-164 verifies app-owned controls and preview-container availability, but Maestro cannot assert rendered row count, colors, alpha, divider insets, Fill/Thumbnail geometry, or clipping. Manually inspect the dedicated Compose previews, including the maximum-widget-font short-landscape case, and validate launcher rendering when host automation is available.
